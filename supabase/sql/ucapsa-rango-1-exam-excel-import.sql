...
declare
  v_batch public.ucapsa_import_batches%rowtype;
begin
  if not public.is_ucapsa_admin() then raise exception 'Solo Admin puede revertir importaciones UCAPSA.'; end if;
  select * into v_batch from public.ucapsa_import_batches where id=p_batch_id for update;
  if not found or v_batch.import_type<>'exam_results' then raise exception 'Lote no encontrado.'; end if;
  if v_batch.status<>'committed' then raise exception 'Solo un lote committed puede revertirse.'; end if;
  perform public.ucapsa_assert_competition_season_mutable(v_batch.season_id);

  if exists (
    select 1 from public.ucapsa_exam_attempts a
    where a.import_batch_id=p_batch_id and (a.status='published' or a.is_official=true)
  ) then
    raise exception 'Un lote con resultados publicados no puede borrarse. Anula/corrige los intentos publicados.'
      using errcode='55000';
  end if;

  delete from public.ucapsa_exam_attempts where import_batch_id=p_batch_id;

  update public.ucapsa_import_batches
  set status='reverted',reverted_at=now(),metadata=metadata || jsonb_build_object('reverted_at',now())
  where id=p_batch_id;

  perform public.ucapsa_exam_admin_audit('ucapsa_exam_import.revert','ucapsa_import_batch',p_batch_id,'{}'::jsonb);
  return p_batch_id;
end;
$$;

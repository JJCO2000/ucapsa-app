...
export default function PaymentsTab() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const [overviewResult, settingsResult] = await Promise.all([getMyPaymentOverview(), getPaymentSettings()]);
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <Text style={[styles.kicker, { color: premium ? '#FFE8B5' : format.accentDark }]}>Mi UCAPSA</Text>
      ...
      {error ? <View style={[styles.errorBox, premium && styles.errorBoxPremium]}><Text style={[styles.errorTitle, premium && styles.errorTitlePremium]}>No se pudieron cargar</Text><Text style={[styles.muted, { color: premium ? '#FFE3E8' : format.muted }]}>{error}</Text></View> : null}

      {!loading && !error ? (
        <>
          <View style={[styles.balanceCard, premium && styles.balanceCardPremium, (overview.attention_total > 0.005 || overview.legacy_membership_pending) && (premium ? styles.balanceCardPremiumPending : styles.balanceCardPending)]}>
            ...
            <Text style={[styles.muted, { color: premium ? '#FFE3E8' : format.muted }]}>
              ...
              {recentObligations.map((item) => (
                <View key={item.id} style={[styles.row, premium && styles.rowPremium]}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.muted, { color: premium ? '#FFE3E8' : format.muted }]}>Vence: {dateLabel(item.due_date)} - {obligationStatusLabel(item)}</Text>
                  </View>
            ...
            {bankReady ? (
              <>
                ...
                <View style={[styles.receiptBox, premium && styles.receiptBoxPremium]}>
                  <MaterialIcons name="chat" size={20} color={premium ? '#FFE8B5' : ucapsaBrand.colors.redDark} />
                  <View style={{ flex: 1 }}>
                ...
                <Pressable style={[styles.whatsappButton, premium && styles.whatsappButtonPremium]} onPress={() => void openWhatsApp()}>
                  <MaterialIcons name="chat" size={19} color={premium ? '#FFE8B5' : ucapsaBrand.colors.redDark} />
              ...
              <Text style={[styles.muted, { color: premium ? '#FFE3E8' : format.muted }]}>Los datos bancarios todavia no estan disponibles. Consulta con UCAPSA antes de transferir.</Text>
            )}
          </View>
          ...
          <View style={[styles.section, premium && styles.sectionPremium]}>
            <View style={styles.sectionHeader}>
              ...
              {overview.payments.length > 0 ? <Pressable onPress={() => router.push('/client/payment-history' as never)}><Text style={[styles.link, { color: premium ? '#FFE8B5' : format.accentDark }]}>Ver historial</Text></Pressable> : null}
            </View>
            {recentPayments.length === 0 ? <Text style={[styles.muted, { color: premium ? '#FFE3E8' : format.muted }]}>Todavia no hay pagos registrados.</Text> : recentPayments.map((payment) => (
              <View key={payment.id} style={[styles.row, premium && styles.rowPremium]}>
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={[styles.muted, { color: premium ? '#FFE3E8' : format.muted }]}>{dateLabel(payment.paid_at)}</Text>
                </View>
    ...
    <View style={[styles.bankRow, premium && styles.rowPremium]}>
      ...
      {copy ? (
        <Pressable accessibilityRole="button" accessibilityLabel={`Copiar ${label}`} style={[styles.smallCopy, premium && styles.smallCopyPremium]} onPress={onCopy}>
          <MaterialIcons name="content-copy" size={18} color={premium ? '#FFE8B5' : ucapsaBrand.colors.redDark} />
        </Pressable>
      ) : null}
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  balanceCardPremium: { borderColor: 'rgba(250,204,21,0.38)', backgroundColor: '#6D0817' },
  balanceCardPremiumPending: { borderColor: '#FACC15', backgroundColor: '#52101A' },
  ...
  sectionPremium: { borderColor: 'rgba(250,204,21,0.34)', backgroundColor: '#38111B' },
  ...
  countPremium: { backgroundColor: 'rgba(250,204,21,0.16)', color: '#FFE8B5' },
  ...
  amountPremium: { color: '#FFE8B5' },
  paidAmount: { color: ucapsaBrand.colors.success, fontSize: 14, fontWeight: '900' },
  paidAmountPremium: { color: '#FACC15' },
  hint: { color: ucapsaBrand.colors.muted, fontSize: 12, fontWeight: '700' },
  bankRow: { flexDirection: 'row', alignItems: 'center', gap: 10, borderTopWidth: 1, borderTopColor: '#F4E5E8', paddingTop: 10 },
  ...
  smallCopyPremium: { backgroundColor: 'rgba(250,204,21,0.14)', borderWidth: 1, borderColor: 'rgba(250,204,21,0.25)' },
  receiptBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, borderRadius: 16, borderWidth: 1, borderColor: '#F7CAD2', backgroundColor: '#FFF3F5', padding: 12, marginTop: 2 },
  receiptBoxPremium: { borderColor: 'rgba(250,204,21,0.28)', backgroundColor: 'rgba(250,204,21,0.08)' },
  ...
  whatsappButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: 16, borderWidth: 1, borderColor: ucapsaBrand.colors.border, backgroundColor: '#fff', paddingVertical: 12 },
  whatsappButtonPremium: { borderColor: 'rgba(250,204,21,0.34)', backgroundColor: 'rgba(255,255,255,0.06)' },
  whatsappButtonText: { color: ucapsaBrand.colors.redDark, fontSize: 13, fontWeight: '900' },
  whatsappButtonTextPremium: { color: '#FFE8B5' },
  secondaryButton: { alignItems: 'center', borderRadius: 16, borderWidth: 1, borderColor: ucapsaBrand.colors.border, paddingVertical: 12 },
  secondaryButtonPremium: { borderColor: 'rgba(250,204,21,0.34)', backgroundColor: 'rgba(255,255,255,0.06)' },

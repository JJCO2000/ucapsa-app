...
function eventToForm(event: UcapsaEvent): EventFormState {
  ...
  return {
    ...
    end_time:
      event.end_date && event.has_time !== false
        ? toTimeValue(event.end_date)
        : "",
...
function isTodayOrFuture(dateValue: string | null | undefined) {
  const key = toDateKey(dateValue);
  if (!key) return true;
  return key >= todayKey();
}

function eventHasCurrentOccurrence(
  event: UcapsaEvent,
  ...
  return occurrences.some(
    (occurrence) =>
      ...
      isTodayOrFuture(occurrence.start_date),
  );
}
...
function validateForm(value: EventFormState) {
  ...
  if (!startDate) {
    throw new Error(
      value.has_time
        ? "Selecciona dia y hora de inicio validos. Usa hora HH:mm."
        : "Selecciona un dia de inicio valido.",
    );
  }

  const endDate = value.has_end_date
    ? buildLocalIso(value.end_day, value.end_time, value.has_time)
    : null;
  if (value.has_end_date && !endDate)
    throw new Error("Selecciona dia y hora de fin validos.");
    ...
    try {
      ...
      await createEvent({
        ...
        repeat_interval_days:
          form.repeat_type === "custom_days" ? repeatInterval : null,
      ...
      Alert.alert(
        ...
        form.repeat_type === "none"
          ? "Se creo 1 evento."
          : "Se guardo como una serie. No se crearon eventos separados.",
      );
    } catch (err) {
      Alert.alert(
        "No se pudo crear",
    ...
    const fakeEvent = {
      ...
      repeat_interval_days:
        form.repeat_type === "custom_days"
          ? Number.parseInt(form.repeat_interval_days || "1", 10)
          : null,
  ...
  async function handleUpdate() {
    ...
    try {
      ...
      await updateEvent(selectedEvent.id, {
        ...
        repeat_interval_days:
          editForm.repeat_type === "custom_days" ? repeatInterval : null,
  ...
  const markedAdminDates = useMemo(() => {
    ...
    for (const occurrence of currentOccurrences) {
      ...
      marks[key] = {
        ...(marks[key] ?? {}),
        marked: true,
        dotColor: occurrence.event.archived_at
          ? "#991b1b"
          : getColorHex(occurrence.event.color_key),
      };
    }
    ...
    return marks;
  }, [currentOccurrences, selectedAdminDay]);

  const currentBaseEvents = useMemo(
    () =>
      events.filter(
        (event) =>
          !event.archived_at && eventHasCurrentOccurrence(event, occurrences),
      ),
    [events, occurrences],
  );
  ...
  const selectedAdminEvents = useMemo<EventOccurrence[]>(
    () =>
      currentOccurrences.filter(
        (occurrence) => toDateKey(occurrence.start_date) === selectedAdminDay,
      ),
    [currentOccurrences, selectedAdminDay],
  );
    ...
    return (
      <View style={styles.deniedContainer}>
        ...
        <Text style={styles.deniedText}>
          Solo administradores pueden gestionar eventos.
        </Text>
      </View>
    );
  }
      ...
      >
        <View style={styles.hero}>
          ...
          <Text style={styles.subtitle}>
            Crea eventos oficiales. Si se repiten, se guardan como una sola
            serie.
          </Text>
        </View>
        ...
        <View style={styles.formCard}>
          ...
          <Text style={styles.muted}>
            Toca un dia para revisar sus eventos u ocurrencias.
          </Text>
        </View>
        ...
        {selectedAdminEvents.length === 0 ? (
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.muted}>
              Puedes crear uno usando el formulario superior.
            </Text>
          </View>
        ) : null}
        ...
        {selectedAdminEvents.map((occurrence) => (
          <View key={occurrence.id} style={styles.adminItem}>
            ...
            <View style={styles.actionsRow}>
              ...
              <ActionButton
                label={
                  occurrence.event.is_published ? "Despublicar" : "Publicar"
                }
                onPress={() =>
                  runAction(() =>
                    setEventPublished(
                      occurrence.event.id,
                      !occurrence.event.is_published,
                    ),
                  )
                }
              />
              <ActionButton
                label={occurrence.event.archived_at ? "Restaurar" : "Archivar"}
                onPress={() =>
                  runAction(() =>
                    occurrence.event.archived_at
                      ? restoreEvent(occurrence.event.id)
                      : archiveEvent(occurrence.event.id),
                  )
                }
              />
              <ActionButton
                ...
                onPress={() =>
                  Alert.alert(
                    "Eliminar evento",
                    "Se eliminara el evento base y todas sus ocurrencias visibles. Esta accion no se puede deshacer.",
                    [
                      { text: "Cancelar", style: "cancel" },
                      {
                        text: "Eliminar",
                        style: "destructive",
                        onPress: () =>
                          runAction(() => deleteEvent(occurrence.event.id)),
                      },
                    ],
                  )
                }
        ...
        <View style={styles.sectionHeader}>
          ...
          <Text style={styles.sectionCount}>{currentBaseEvents.length}</Text>
        </View>

        {currentBaseEvents.slice(0, 10).map((event) => (
          <EventCard
            key={`base-${event.id}`}
            event={event}
              ...
              <ScrollView contentContainerStyle={styles.modalContent}>
                ...
                {selectedEvent ? (
                  <View style={styles.modalActionsGrid}>
                    <ActionButton
                      label={
                        selectedEvent.is_published ? "Despublicar" : "Publicar"
                      }
                      onPress={() =>
                        runAction(
                          () =>
                    ...
                    <ActionButton
                      label={
                        selectedEvent.archived_at ? "Restaurar" : "Archivar"
                      }
                      onPress={() =>
                        runAction(
                          () =>
                            selectedEvent.archived_at
                              ? restoreEvent(selectedEvent.id)
                              : archiveEvent(selectedEvent.id),
                          true,
                        )
                      }
                    />
                    <ActionButton
                      ...
                      onPress={() =>
                        Alert.alert(
                          "Eliminar evento",
                          "Esta accion eliminara la serie completa si el evento se repite.",
                          [
                            { text: "Cancelar", style: "cancel" },
                            {
                              text: "Eliminar",
...
function EventForm({
  ...
  const selectedRepeat =
    repeatOptions.find((option) => option.value === form.repeat_type)?.label ??
    "No se repite";
  const selectedColor =
    colorOptions.find((option) => option.value === form.color_key)?.label ??
    "Verde";
  const selectedPriority =
    priorityOptions.find((option) => option.value === form.priority)?.label ??
    "Normal";
  ...
  return (
    <View style={styles.formFields}>
      ...
      <Pressable
        ...
        onPress={() => setShowStartCalendar((value) => !value)}
      >
        <Text style={styles.dropdownText}>{form.start_day}</Text>
        <MaterialIcons
          name={showStartCalendar ? "expand-less" : "expand-more"}
          size={24}
          color="#0f766e"
        />
      </Pressable>
        ...
        <View style={styles.calendarMiniCard}>
          <Calendar
            ...
            onDayPress={(day: DateData) => {
              onChange({
                ...form,
                start_day: day.dateString,
                end_day: form.has_end_date ? form.end_day : day.dateString,
              });
      ...
      <View style={styles.switchRow}>
        ...
        <Switch
          value={form.has_time}
          onValueChange={(has_time) =>
            onChange({
              ...form,
              has_time,
              start_time: has_time ? form.start_time || "10:00" : "",
              end_time: has_time ? form.end_time || "11:00" : "",
            })
          }
        />
      </View>
      ...
      {form.has_time ? (
        <TextInput
          ...
          onChangeText={(start_time) => onChange({ ...form, start_time })}
          placeholder="Hora, ejemplo 10:00"
      ...
      <View style={styles.switchRow}>
        ...
        <Switch
          value={form.has_end_date}
          onValueChange={(has_end_date) =>
            onChange({
              ...form,
              has_end_date,
              end_day: has_end_date ? form.end_day : form.start_day,
            })
          }
        />
      </View>
      ...
      {form.has_end_date ? (
        <>
          ...
          <Pressable
            style={styles.dropdownButton}
            onPress={() => setShowEndCalendar((value) => !value)}
          >
            <Text style={styles.dropdownText}>{form.end_day}</Text>
            <MaterialIcons
              name={showEndCalendar ? "expand-less" : "expand-more"}
              size={24}
              color="#0f766e"
            />
          </Pressable>
          ...
          {form.has_time ? (
            <TextInput
              value={form.end_time}
              onChangeText={(end_time) => onChange({ ...form, end_time })}
              placeholder="Hora de fin, ejemplo 12:00"
              keyboardType="numbers-and-punctuation"
              style={styles.input}
            />
          ) : null}
      ...
      <View style={styles.segmentRow}>
        {audienceOptions.map((option) => (
          <Pressable
            ...
            style={[
              styles.segment,
              form.audience === option.value && styles.segmentActive,
            ]}
          >
            <Text
              style={[
                styles.segmentText,
                form.audience === option.value && styles.segmentTextActive,
              ]}
            >
              {option.label}
            </Text>
      ...
      <Pressable
        style={styles.dropdownButton}
        onPress={() => {
          setShowColorOptions((value) => !value);
      ...
      >
        ...
        <MaterialIcons
          name={showColorOptions ? "expand-less" : "expand-more"}
          size={24}
          color="#0f766e"
        />
      </Pressable>
        ...
        <View style={styles.optionList}>
          {colorOptions.map((option) => (
            <Pressable
              ...
              style={[
                styles.optionItem,
                form.color_key === option.value && styles.optionItemActive,
              ]}
              onPress={() => {
                onChange({ ...form, color_key: option.value });
            ...
            >
              <Text
                style={[
                  styles.optionText,
                  form.color_key === option.value && styles.optionTextActive,
                ]}
              >
                {option.label}
              </Text>
      ...
      <Pressable
        style={styles.dropdownButton}
        onPress={() => {
          setShowPriorityOptions((value) => !value);
          setShowColorOptions(false);
          setShowRepeatOptions(false);
        }}
      >
        ...
        <MaterialIcons
          name={showPriorityOptions ? "expand-less" : "expand-more"}
          size={24}
          color="#0f766e"
        />
      </Pressable>
        ...
        <View style={styles.optionList}>
          {priorityOptions.map((option) => (
            <Pressable
              ...
              style={[
                styles.optionItem,
                form.priority === option.value && styles.optionItemActive,
              ]}
              onPress={() => {
                onChange({ ...form, priority: option.value });
                setShowPriorityOptions(false);
              }}
            >
              <Text
      ...
      {allowRepeat ? (
        <View style={styles.repeatBox}>
          ...
          <Pressable
            style={styles.dropdownButton}
            onPress={() => setShowRepeatOptions((value) => !value)}
          >
            <Text style={styles.dropdownText}>{selectedRepeat}</Text>
            <MaterialIcons
              name={showRepeatOptions ? "expand-less" : "expand-more"}
              size={24}
              color="#0f766e"
            />
          </Pressable>
          ...
          {form.repeat_type === "custom_days" ? (
            <TextInput
              ...
              onChangeText={(repeat_interval_days) =>
                onChange({ ...form, repeat_interval_days })
              }
              placeholder="Cada cuantos dias, ejemplo 20"
          ...
          {form.repeat_type !== "none" ? (
            <Text style={styles.muted}>
              Se guardara como una sola serie. No se crean eventos separados.
              Maximo 10 ocurrencias visibles.
            </Text>
          ) : null}
        </View>
      ) : null}
...
function ActionButton({
  ...
  return (
    ...
    >
      <Text
        style={[styles.actionButtonText, danger && styles.dangerButtonText]}
      >
        {label}
      </Text>
    </Pressable>

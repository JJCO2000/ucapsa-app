...
export default function AccountSettingsScreen() {
  ...
  const profileComplete = Boolean((profile?.full_name ?? '').trim() && (profile?.phone ?? '').trim() && (isAdmin || (profile?.dog_name ?? '').trim()));
  ...
  if (!user) {
    return (
      <KeyboardAwareScreen style={{ backgroundColor: ucapsaBrand.colors.background }}>
        <Header title="Ajustes" subtitle="Inicia sesion para gestionar tus datos y notificaciones." onBack={() => router.back()} premium={false} />
  ...
  return (
    ...
    >
      <Header
        ...
        subtitle={isAdmin ? 'Estadisticas, datos y notificaciones sin saturar Perfil.' : 'Tus datos, notificaciones y solicitud de cuenta.'}
        onBack={() => router.back()}
        premium={isPremium}
      />
      ...
      <View style={[styles.identityCard, isPremium && styles.premiumCard]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={[styles.identityName, isPremium && styles.premiumTitle]}>{fullName || profile?.email || 'Usuario UCAPSA'}</Text>
          <Text style={[styles.identityEmail, isPremium && styles.premiumText]}>{profile?.email ?? user.email}</Text>
          <Text style={[styles.identityRole, isPremium && styles.premiumPill]}>Rol: {role ?? 'client'}</Text>
        </View>
      </View>
      ...
      {isAdmin ? (
        ...
        >
          <View style={styles.statsGrid}>
            <AdminStat label="Clientes" value={adminStats.clients} icon="account" onPress={() => router.push('/admin/users?filter=clients_and_members' as never)} />
            <AdminStat label="Socios" value={adminStats.members} icon="badge-account" onPress={() => router.push('/admin/users?filter=members' as never)} />
            <AdminStat label="Membresias activas" value={adminStats.activeMemberships} icon="check-decagram" onPress={() => router.push('/admin/members?filter=active' as never)} />
            <AdminStat label="Falta pago" value={adminStats.pendingPayments} icon="cash-remove" onPress={() => router.push('/admin/members?filter=payment_pending_this_month' as never)} />
            <AdminStat label="Solicitudes" value={adminStats.pendingRequests} icon="email-outline" onPress={() => router.push('/admin/members?filter=pending_requests' as never)} />
            <AdminStat label="Vigencia vencida" value={adminStats.expiredMemberships} icon="calendar-alert" onPress={() => router.push('/admin/members?filter=expired_by_date' as never)} />
            <AdminStat label="Anuncios" value={adminStats.announcements} icon="bullhorn" onPress={() => router.push('/admin/announcements' as never)} />
            <AdminStat label="Eventos" value={adminStats.events} icon="calendar-month" onPress={() => router.push('/admin/events' as never)} />
          </View>
        </AccordionSection>
      ) : null}
      ...
      <AccordionSection
        ...
        subtitle={profileComplete ? 'Nombre, contacto y avatar.' : 'Completa tu informacion de contacto.'}
        icon="person"
      ...
      >
        <View style={styles.sectionHeaderRow}>
          <View style={{ flex: 1 }}>
            <Text style={[styles.eyebrow, isPremium && styles.premiumEyebrow]}>Cuenta</Text>
            <Text style={[styles.sectionTitle, isPremium && styles.premiumTitle]}>{isAdmin ? 'Mis datos administrativos' : 'Mis datos'}</Text>
          </View>
          <Pressable style={[styles.smallButton, isPremium && styles.premiumSmallButton]} onPress={() => setEditing(true)}>
            <Text style={[styles.smallButtonText, isPremium && styles.premiumSmallButtonText]}>{profileComplete ? 'Editar' : 'Completar'}</Text>
          </Pressable>
        </View>

        {!profileComplete ? (
          <Text style={[styles.warningText, isPremium && styles.premiumText]}>Completa estos datos para que UCAPSA tenga informacion correcta de contacto.</Text>
        ) : null}

        <ReadonlyRow label="Nombre" value={profile?.full_name || 'Pendiente'} premium={isPremium} />
        <ReadonlyRow label="Correo" value={profile?.email || user.email || 'Pendiente'} premium={isPremium} />
        <ReadonlyRow label="Telefono" value={profile?.phone || 'Pendiente'} premium={isPremium} />
        {!isAdmin ? <ReadonlyRow label="Perro" value={profile?.dog_name || 'Pendiente'} premium={isPremium} /> : null}
      </AccordionSection>

      {!isAdmin ? (
        <AccordionSection
          title="Eliminar cuenta"
          subtitle={deletionRequested ? 'Solicitud registrada.' : 'Crear solicitud para administracion.'}
          icon="delete-outline"
        ...
        >
          ...
          <Pressable disabled={deletionRequested} style={[styles.dangerButton, isPremium && styles.premiumDangerButton, deletionRequested && styles.disabled]} onPress={handleDeleteRequest}>
            <Text style={[styles.dangerButtonText, isPremium && styles.premiumDangerButtonText]}>{deletionRequested ? 'Eliminacion solicitada' : 'Solicitar eliminacion de cuenta'}</Text>
          </Pressable>
        </AccordionSection>
      ) : null}

      <Pressable style={[styles.secondaryButton, isPremium && styles.premiumSecondaryButton]} onPress={handleSignOut}>
        ...
        <View style={styles.modalBackdrop}>
          <View style={[styles.modalCard, isPremium && styles.premiumCard]}>
            <View style={styles.modalHeader}>
              <Text style={[styles.modalTitle, isPremium && styles.premiumTitle]}>{isAdmin ? 'Editar datos administrativos' : 'Editar mis datos'}</Text>
            ...
            <Pressable disabled={saving} style={[styles.primaryButton, isPremium && styles.premiumPrimaryButton, saving && styles.disabled]} onPress={handleSaveProfile}>
              <Text style={[styles.primaryButtonText, isPremium && styles.premiumPrimaryButtonText]}>{saving ? 'Guardando...' : 'Guardar cambios'}</Text>
            </Pressable>
          </View>
        </View>
      </Modal>
...
function Header({ title, subtitle, onBack, premium }: { title: string; subtitle: string; onBack: () => void; premium: boolean }) {
  return (
    <View style={styles.headerRow}>
      <Pressable accessibilityRole="button" accessibilityLabel="Volver" style={[styles.backButton, premium && styles.premiumSmallButton]} onPress={onBack}>
        <MaterialIcons name="arrow-back" size={22} color={premium ? '#FFE8B5' : ucapsaBrand.colors.redDark} />
      </Pressable>
      <View style={{ flex: 1 }}>
        <Text style={[styles.title, premium && styles.premiumTitle]}>{title}</Text>
        <Text style={[styles.muted, premium && styles.premiumText]}>{subtitle}</Text>
      </View>
    </View>
  );
}
...
function AccordionSection({
  ...
  return (
    <View style={[styles.accordionCard, premium && styles.premiumCard, danger && styles.dangerAccordionCard]}>
      <Pressable accessibilityRole="button" accessibilityLabel={`${open ? 'Cerrar' : 'Abrir'} ${title}`} style={styles.accordionHeader} onPress={onPress}>
        <View style={[styles.accordionIcon, premium && styles.premiumSmallButton, danger && styles.dangerIcon]}>
          <MaterialIcons name={icon} size={21} color={premium ? '#FFE8B5' : danger ? '#C43B4E' : ucapsaBrand.colors.redDark} />
        </View>
        <View style={{ flex: 1 }}>
          <View style={styles.accordionTitleRow}>
            <Text style={[styles.accordionTitle, premium && styles.premiumTitle, danger && styles.dangerAccordionTitle]}>{title}</Text>
            {badge ? <Text style={[styles.pendingBadge, premium && styles.premiumPill]}>{badge}</Text> : null}
          </View>
          <Text style={[styles.accordionSubtitle, premium && styles.premiumText]}>{subtitle}</Text>
        </View>
        <MaterialIcons name={open ? 'expand-less' : 'expand-more'} size={24} color={premium ? '#FFE8B5' : ucapsaBrand.colors.muted} />
      </Pressable>
...
function AdminStat({ label, value, icon, onPress }: { label: string; value: number; icon: keyof typeof MaterialCommunityIcons.glyphMap; onPress: () => void }) {
  return (
...
const styles = StyleSheet.create({
  ...
  muted: { color: ucapsaBrand.colors.muted, fontSize: 14, lineHeight: 20, marginTop: 4 },
  identityCard: { flexDirection: 'row', alignItems: 'center', gap: 14, backgroundColor: '#fff', borderRadius: 28, borderWidth: 1, borderColor: ucapsaBrand.colors.border, padding: 16 },
  ...
  accordionCard: { backgroundColor: '#fff', borderRadius: 22, borderWidth: 1, borderColor: ucapsaBrand.colors.border, overflow: 'hidden' },
  accordionHeader: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14 },
  accordionIcon: { width: 38, height: 38, borderRadius: 19, backgroundColor: ucapsaBrand.colors.redSoft, alignItems: 'center', justifyContent: 'center' },
  accordionTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' },
  accordionTitle: { color: ucapsaBrand.colors.text, fontSize: 17, fontWeight: '900' },
  accordionSubtitle: { color: ucapsaBrand.colors.muted, fontSize: 12, fontWeight: '700', lineHeight: 17, marginTop: 2 },
  accordionBody: { paddingHorizontal: 14, paddingBottom: 14, gap: 10 },
  pendingBadge: { overflow: 'hidden', backgroundColor: '#FFF7ED', color: '#9A3412', borderRadius: 999, paddingHorizontal: 9, paddingVertical: 4, fontSize: 11, fontWeight: '900' },
  dangerAccordionCard: { borderColor: '#F4B8C1' },
  dangerAccordionTitle: { color: '#C43B4E' },
  dangerIcon: { backgroundColor: '#FFF1F3' },
  card: { backgroundColor: '#fff', borderRadius: 26, borderWidth: 1, borderColor: ucapsaBrand.colors.border, padding: 16, gap: 10 },
  ...
  sectionTitle: { color: ucapsaBrand.colors.text, fontSize: 21, fontWeight: '900', marginTop: 2 },
  warningText: { color: ucapsaBrand.colors.warning, fontSize: 13, fontWeight: '800', lineHeight: 19, marginTop: 4 },
  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 8 },
  statCard: { width: '48%', backgroundColor: ucapsaBrand.colors.surfaceAlt, borderRadius: 20, borderWidth: 1, borderColor: ucapsaBrand.colors.border, padding: 12 },
  statIconWrap: { width: 34, height: 34, borderRadius: 17, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' },
  statValue: { color: ucapsaBrand.colors.text, fontSize: 24, fontWeight: '900', marginTop: 10 },
  statLabel: { color: ucapsaBrand.colors.muted, fontSize: 12, fontWeight: '800', marginTop: 4 },
  ...
  secondaryButton: { backgroundColor: '#fff', borderWidth: 1, borderColor: ucapsaBrand.colors.border, borderRadius: 18, paddingVertical: 14, alignItems: 'center', marginTop: 4 },
  secondaryButtonText: { color: ucapsaBrand.colors.text, fontSize: 15, fontWeight: '900' },
  dangerButton: { backgroundColor: '#FFF1F3', borderWidth: 1, borderColor: '#F4B8C1', borderRadius: 18, paddingVertical: 14, alignItems: 'center', marginTop: 8 },
  ...
  modalCard: { backgroundColor: ucapsaBrand.colors.background, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 20, gap: 10, borderWidth: 1, borderColor: ucapsaBrand.colors.border },
  modalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 },
  modalTitle: { color: ucapsaBrand.colors.text, fontSize: 21, fontWeight: '900' },
  label: { color: ucapsaBrand.colors.text, fontSize: 13, fontWeight: '800', marginBottom: 6, marginTop: 10 },
  ...
  colorRow: { flexDirection: 'row', gap: 10, marginTop: 4, marginBottom: 8, flexWrap: 'wrap' },
  ...
  premiumSmallButton: { backgroundColor: 'rgba(250,204,21,0.16)', borderWidth: 1, borderColor: 'rgba(250,204,21,0.34)' },
  ...
  premiumDangerButton: { backgroundColor: 'rgba(255,255,255,0.05)', borderColor: 'rgba(250,204,21,0.34)' },
